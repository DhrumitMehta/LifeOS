...
const FinanceScreen = () => {
  ...
  const getNetWorth = () => {
    // Net worth = Cash balance = Income - Expenses
    // Investments are already accounted for in income/expense transactions
    return getTotalIncome() - getTotalExpenses();
  };

  const getCashBalance = () => {
    const cashTransactions = state.transactions.filter(t => t.account === 'Cash');
    const income = cashTransactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
    const expenses = cashTransactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    return income - expenses;
  };

  const getBankBalance = () => {
    const bankTransactions = state.transactions.filter(t => t.account === 'Bank');
    const income = bankTransactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
    const expenses = bankTransactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    return income - expenses;
  };

  const getMobileBalance = () => {
    const mobileTransactions = state.transactions.filter(t => t.account === 'Mobile');
    const income = mobileTransactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
    const expenses = mobileTransactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
    return income - expenses;
  };
    ...
    >
      <View style={styles.summaryCards}>
        ...
        <Card style={[styles.summaryCard, { backgroundColor: '#3b82f6' }]}>
          <Card.Content>
            <Title style={styles.summaryTitle}>Bank Balance</Title>
            <Text style={styles.summaryAmount}>{formatCurrency(getBankBalance())}</Text>
          </Card.Content>
        </Card>

        <Card style={[styles.summaryCard, { backgroundColor: '#10b981' }]}>
          <Card.Content>
            <Title style={styles.summaryTitle}>Mobile Money</Title>
        ...
        <Card style={[styles.summaryCard, { backgroundColor: '#8b5cf6' }]}>
          <Card.Content>
            <Title style={styles.summaryTitle}>Total Balance</Title>

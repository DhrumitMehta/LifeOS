  ...
  const handleSaveHabit = async () => {
    ...
    try {
      ...
      color: '#3b82f6',
        icon: 'check',
      isActive: true,
        notificationTime: newHabit.notificationsEnabled ? newHabit.notificationTime : undefined,
        notificationsEnabled: newHabit.notificationsEnabled,
      };
      
      const habit = await addHabit(habitData);

      // Schedule notification if enabled
      if (newHabit.notificationsEnabled && newHabit.notificationTime) {
        await NotificationService.scheduleHabitReminder(habit, newHabit.notificationTime);
      }
          ...
          <Dialog.Content>
            ...
            <View style={styles.notificationContainer}>
              ...
              <View style={styles.notificationRow}>
                <Checkbox
                  status={newHabit.notificationsEnabled ? 'checked' : 'unchecked'}
                  onPress={() => setNewHabit({ ...newHabit, notificationsEnabled: !newHabit.notificationsEnabled })}
                />
                <Text style={styles.notificationText}>Enable daily reminders</Text>
              </View>
              {newHabit.notificationsEnabled && (
                <TextInput
                  label="Reminder Time (HH:MM)"
                  value={newHabit.notificationTime}
                  onChangeText={(text) => setNewHabit({ ...newHabit, notificationTime: text })}
                  style={styles.input}
                  placeholder="09:00"
                />
              )}

...
const HabitsScreen = () => {
  ...
  const handleHabitToggle = async (habit: Habit, date: Date) => {
    ...
    if (isCompleted) {
      // If already completed, we could implement un-complete functionality
      Alert.alert('Already Completed', 'This habit is already completed for this day.');
      return;
    }
  ...
  const renderGridHeader = () => (
    <View style={styles.gridHeader}>
      ...
      {state.habits.filter(habit => habit.isActive).map(habit => (
        ...
        >
          <Text style={[styles.headerText, { color: habit.color, textDecorationLine: 'underline' }]} numberOfLines={2}>
            {habit.name}
          </Text>
        </TouchableOpacity>
      ))}
  ...
  const renderDayRow = (date: Date) => (
    <View key={date.toISOString()} style={styles.dayRow}>
      ...
      {state.habits.filter(habit => habit.isActive).map(habit => (
        <View key={habit.id} style={styles.habitColumn}>
          <Checkbox
    ...
    <View style={styles.container}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={styles.gridContainer}>
          {renderGridHeader()}
          {last7Days.map(renderDayRow)}
        </View>
      </ScrollView>

      {/* Streaks and Analytics Section */}
      <ScrollView style={styles.analyticsSection}>
        {state.habits.filter(habit => habit.isActive).map(habit => {
          const streak = getHabitStreak(habit);
          const progress = getHabitProgress(habit);
          
          return (
            <Card key={habit.id} style={styles.analyticsCard}>
              <Card.Content>
                <View style={styles.analyticsHeader}>
                  <View style={styles.habitInfo}>
                    <Title style={[styles.habitName, { color: habit.color }]}>
                      {habit.name}
                    </Title>

...
const FinanceScreen = () => {
  ...
  // Calculate trend (increase/decrease) for a time period
  const getTrendForPeriod = (category: string, monthsBack: number) => {
    if (!category) return null;
    
    const current = getExpenseForPeriod(category, monthsBack);
    const previous = getExpenseForPeriod(category, monthsBack * 2);
  ...
  const categoryTrendData = useMemo(() => {
    ...
    return periods.map(period => {
      const amount = getExpenseForPeriod(selectedCategory, period.months);
      const trend = getTrendForPeriod(selectedCategory, period.months);
              ...
              {categoryTrendData.map((data, index) => (
                <View key={index} style={styles.trendItem}>
                  <View style={styles.trendInfo}>
                    ...
                    <Text style={styles.trendAmount}>{formatCurrency(data.amount)}</Text>
                  </View>
                  {data.trend !== null && (
                    <View style={[
                      styles.trendBadge,
                      { backgroundColor: data.trend > 0 ? '#fee2e2' : '#d1fae5' }
                    ]}>
                      <Ionicons
                        name={data.trend > 0 ? 'trending-up' : 'trending-down'}
                        size={16}
                        color={data.trend > 0 ? '#dc2626' : '#059669'}
                      />
                      <Text style={[
                        styles.trendText,
                        { color: data.trend > 0 ? '#dc2626' : '#059669' }

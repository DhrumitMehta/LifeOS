...
async function extractInvestments() {
  try {
    ...
    investmentTransactions.forEach((transaction: any) => {
      ...
      // Determine fund based on description
      if (desc.includes('utt') && desc.includes('amoja')) {
        ...
        fundType = 'mutual-fund';
      } else if (desc.includes('quiver') || desc.includes('15%')) {
        fundName = 'Quiver 15% Fund';
        fundType = 'mutual-fund';
      } else if (desc.includes('investment') || desc.includes('fund')) {
        fundName = 'Other Investment Fund';
        fundType = 'mutual-fund';
      }

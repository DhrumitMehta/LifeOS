...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshData = async () => {
    try {
      ...
      dispatch({ type: 'SET_HABITS', payload: habits as Habit[] });

      // Load habit entries
      const habitEntries = await db.getAllAsync('SELECT * FROM habit_entries ORDER BY date DESC');
      dispatch({ type: 'SET_HABIT_ENTRIES', payload: habitEntries as HabitEntry[] });

      // Load journal entries
      const journalEntries = await db.getAllAsync('SELECT * FROM journal_entries ORDER BY date DESC');
      dispatch({ type: 'SET_JOURNAL_ENTRIES', payload: journalEntries as JournalEntry[] });
      ...
      // Load investments
      const investments = await db.getAllAsync('SELECT * FROM investments ORDER BY created_at DESC');
      dispatch({ type: 'SET_INVESTMENTS', payload: investments as Investment[] });
      ...
      // Load accounts
      const accounts = await db.getAllAsync('SELECT * FROM accounts ORDER BY created_at DESC');
      dispatch({ type: 'SET_ACCOUNTS', payload: accounts as Account[] });
    } catch (error) {

...
try {
  ...
  // Extract URL and key using regex (simple approach)
  ...
  console.log('Could not read supabase config, trying example file...');
}
...
if (!supabaseUrl || !supabaseKey) {
  try {
    ...
    const urlMatch = exampleContent.match(/supabaseUrl:\s*['"](.+?)['"]/);
    const keyMatch = exampleContent.match(/supabaseKey:\s*['"](.+?)['"]/);
...
function formatTransaction(transaction: Transaction) {
  return {
    ...
    date: transaction.date.toISOString().split('T')[0], // Date only

...
console.log(`\nRows that might have been skipped (have date+desc+category but no amounts):`);
...
skippedRows.forEach((row: any, idx: number) => {
  ...
  console.log(`     Source: ${row.source}, CashBal: ${row.cashBalance}, BankBal: ${row.bankBalance}, MobileBal: ${row.mobileBalance}`);
});

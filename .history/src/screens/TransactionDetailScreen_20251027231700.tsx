...
const TransactionDetailScreen = () => {
  ...
  const accounts = ['Cash', 'NMB Main A/C', 'Selcom', 'NMB Virtual Card', 'Mobile', 'Inv'];

  // Auto-select subcategory based on the selected date (only when creating new transaction or when subcategory is empty)
  useEffect(() => {
    if (isEditing && !transaction) {
      const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
      const monthName = monthNames[formData.date.getMonth()];
      setFormData(prevData => ({
        ...prevData,
        subcategory: monthName
      }));
    }
  ...
  const handleSave = async () => {
    ...
    const combinedAmount = parseFloat(`${amountWhole}.${amountDecimal}`);
        ...
        <Card style={styles.card}>
          <Card.Content>
            ...
            <View style={styles.amountContainer}>
              <TextInput
                ...
                onChangeText={(text) => {
                  ...
                  setAmountWhole(digits || '0');
                }}
                keyboardType="number-pad"
                style={[styles.input, styles.amountWholeInput]}
              />
              ...
              <TextInput
                ...
                onChangeText={(text) => {
                  ...
                  const digits = text.replace(/[^0-9]/g, '').substring(0, 2);
                  setAmountDecimal(digits || '00');
                }}
                keyboardType="number-pad"
                style={[styles.input, styles.amountDecimalInput]}
              />
            </View>
            ...
            <TextInput
              ...
              editable={false}
              style={styles.input}
              helperText="Automatically set based on selected date"
            />
      ...
      {showDatePicker && (
        <View style={styles.datePickerContainer}>
          <Card style={styles.datePickerCard}>
            <Card.Content>
              <Calendar
                current={formData.date.toISOString().split('T')[0]}
                onDayPress={(day) => {
                ...
                markedDates={{
                  [formData.date.toISOString().split('T')[0]]: { selected: true, selectedColor: '#6366f1' }
                }}
                theme={{

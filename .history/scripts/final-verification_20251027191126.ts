...
async function verify() {
  ...
  allTransactions
    ...
    .forEach((t: any) => {
      const account = t.account as keyof typeof balances;
      if (t.type === 'income') {
        balances[account].income += parseFloat(t.amount);
        balances[account].balance += parseFloat(t.amount);
      } else {
        balances[account].expense += parseFloat(t.amount);
        balances[account].balance -= parseFloat(t.amount);
      }
    });

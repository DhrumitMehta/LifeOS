          ...
          <View key={habit.id} style={[styles.habitColumn, isToday && styles.todayHabitColumn]}>
            {habit.habitType === 'boolean' ? (
              ...
              <View style={styles.numericInputContainer}>
                <TextInput
                  value={numericValues[`${habit.id}-${date.toISOString().split('T')[0]}`] || ''}
                  onChangeText={(text) => {
                    const key = `${habit.id}-${date.toISOString().split('T')[0]}`;
                    setNumericValues({...numericValues, [key]: text});
                  }}
                  onBlur={() => handleNumericHabitChange(habit, date)}
                  keyboardType="numeric"
                  style={styles.numericInput}
                  placeholder="0"
                />
                <Text style={styles.numericUnit}>{habit.unit}</Text>
              </View>
            )}
          </View>
        ))}

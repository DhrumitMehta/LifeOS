...
data.forEach((row: any, index: number) => {
  ...
  // Skip balance-setting entries (Cash Balance, Bank Balance initial entries)
  if (row.description && 
      ((row.description.includes('Cash Balance') && row.income === 71000) ||
       (row.description.includes('Bank Balance') && row.income === 345000))) {
    return;
  }
  ...
  if (row.income && row.income !== 0) {
    type = 'income';
    amount = Math.abs(row.income); // Ensure positive amount
  } else if (row.expenses && row.expenses !== 0) {
    // Negative expenses are corrections (should be income)
    if (row.expenses < 0) {
      type = 'income';

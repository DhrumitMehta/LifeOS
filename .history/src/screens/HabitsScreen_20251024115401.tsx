...
const HabitsScreen = () => {
  ...
  const handleNumericHabitChange = async (habit: Habit, date: Date) => {
    try {
      ...
      if (existingEntry) {
        ...
        await addHabitEntry({
          ...
          date: date,
          value: numericValue,
          completed: numericValue >= habit.targetValue,
  ...
  const handleSaveHabit = async () => {
    ...
    try {
      ...
      setNewHabit({ name: '', description: '', category: 'Health', habitType: 'boolean', targetValue: 1, unit: 'times' });
    } catch (error) {
          ...
          <View key={habit.id} style={[styles.habitColumn, isToday && styles.todayHabitColumn]}>
            {habit.habitType === 'boolean' ? (
              ...
              <View style={styles.numericInputContainer}>
                <Slider
                  value={parseFloat(numericValues[`${habit.id}-${date.toISOString().split('T')[0]}`] || '0')}
                  onValueChange={(value) => {
                    const key = `${habit.id}-${date.toISOString().split('T')[0]}`;
                    setNumericValues({...numericValues, [key]: value.toString()});
                  }}
                  onSlidingComplete={() => handleNumericHabitChange(habit, date)}
                  minimumValue={0}
                  maximumValue={habit.maxValue || 10}
                  step={habit.maxValue && habit.maxValue <= 10 ? 1 : 0.1}
                  style={styles.numericSlider}
                  thumbStyle={{ backgroundColor: habit.color }}
                  trackStyle={{ backgroundColor: '#e0e0e0' }}
                />
                <Text style={styles.numericValue}>
                  {numericValues[`${habit.id}-${date.toISOString().split('T')[0]}`] || '0'} {habit.unit}
                </Text>
              </View>
            )}
          </View>
                ...
                <View style={styles.habitDetailInfo}>
                  ...
                  <Text style={styles.habitDetailValue}>
                    {selectedHabit.habitType === 'boolean' 
                      ...
                      : `${selectedHabit.targetValue} ${selectedHabit.unit} per day`
                    }
                  </Text>
                </View>
                  ...
                  <View style={styles.progressInfo}>
                    <Text style={styles.progressText}>
                      {selectedHabit.habitType === 'boolean' 
                        ...
                        : `${getHabitCompletionRate(selectedHabit)}% Target Met`
                      }
                    </Text>
                  </View>
                  <ProgressBar

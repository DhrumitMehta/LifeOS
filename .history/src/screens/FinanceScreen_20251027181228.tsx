...
const FinanceScreen = () => {
  ...
  const getNetWorth = () => {
    // Get investment purchase expenses (these shouldn't reduce net worth since the money went into investments)
    const investmentExpenses = state.transactions
      .filter(t => t.type === 'expense' && t.category === 'Investment')
      .reduce((sum, t) => sum + t.amount, 0);
    
    // Net worth = (Income - Expenses) + Investment Values
    // But subtract investment purchase expenses since investments already include their value
    const adjustedExpenses = getTotalExpenses() - investmentExpenses;
    return getTotalIncome() - adjustedExpenses + getTotalInvestments();
  };

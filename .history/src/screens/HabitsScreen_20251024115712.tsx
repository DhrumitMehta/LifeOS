            ...
            {habit.habitType === 'boolean' ? (
              ...
              <View style={styles.numericInputContainer}>
                <View style={styles.numericControls}>
                  <TouchableOpacity
                    style={[styles.numericButton, { backgroundColor: habit.color }]}
                    onPress={() => {
                      const key = `${habit.id}-${date.toISOString().split('T')[0]}`;
                      const currentValue = parseFloat(numericValues[key] || '0');
                      const newValue = Math.max(0, currentValue - (habit.maxValue && habit.maxValue <= 10 ? 1 : 0.1));
                      setNumericValues({...numericValues, [key]: newValue.toString()});
                      handleNumericHabitChange(habit, date);
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      const key = `${habit.id}-${date.toISOString().split('T')[0]}`;
                      const currentValue = parseFloat(numericValues[key] || '0');
                      const maxValue = habit.maxValue || 10;
                      const step = maxValue <= 10 ? 1 : 0.1;

...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const deleteHabitEntry = async (entryId: string) => {
    try {
      console.log('Context: Deleting habit entry:', entryId);
      
      // Use individual delete method
      await supabaseDatabase.deleteHabitEntry(entryId);
      
      // Update local state
      dispatch({ type: 'DELETE_HABIT_ENTRY', payload: entryId });
      
      console.log('Context: Habit entry deleted successfully');
    } catch (error) {
      console.error('Context: Error deleting habit entry:', error);
      throw error;
    }
  };

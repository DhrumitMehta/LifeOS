...
import { getFundPricesByName, FundProvider } from '../services/mutualFundService';
...
const InvestmentDetailScreen = () => {
  ...
  const handleFetchPrices = async () => {
    ...
    setLoading(true);
    try {
      const prices = await getFundPricesByName(formData.name, formData.provider);
      if (prices) {
            ...
            {formData.type === 'mutual-fund' ? (
              <>
                ...
                <SegmentedButtons
                  ...
                  onValueChange={(value) => setFormData({ ...formData, provider: value as FundProvider })}
                  buttons={[
                    { value: 'UTT', label: 'UTT' },

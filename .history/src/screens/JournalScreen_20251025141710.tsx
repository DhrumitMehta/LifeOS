...
const JournalScreen = () => {
  ...
  const getHighlightedPreview = (entry: JournalEntry) => {
    ...
    const structuredFields = [
      ...
      entry.affirmations,
      entry.openThoughts
    ].filter(field => field && field.toLowerCase().includes(query));
    
    if (structuredFields.length > 0) {
      const matchedField = structuredFields[0];
      const fieldPreview = matchedField.length > 100 
        ? matchedField.substring(0, 100) + '...' 
        : matchedField;
      ...
      return (
        <View>
          ...
          <Text style={styles.structuredMatch}>
            💭 {highlightText(fieldPreview, searchQuery)}
          </Text>
        </View>
      );
    }
    ...
    >
      ...
      {isEditing ? (
        ...
        <>
                ...
                >
                  <Card style={styles.entryCard}>
                    <Card.Content>
                      ...
                      {/* Show structured fields preview if they exist */}
                      {(entry.memorableMoment || entry.affirmations || entry.openThoughts) && (
          ...
          {/* No results message */}
          {filteredEntries.length === 0 && (searchQuery || searchFilters.mood || searchFilters.dateFrom || searchFilters.dateTo) && (

...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addHabitEntry = async (entryData: Omit<HabitEntry, 'id' | 'createdAt'>) => {
    ...
    await performCRUD(
      database.getHabitEntries,
      database.saveHabitEntries,
      { type: 'ADD_HABIT_ENTRY', payload: entry },
  ...
  const addJournalEntry = async (entryData: Omit<JournalEntry, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    await performCRUD(
      database.getJournalEntries,
      database.saveJournalEntries,
  ...
  const addInvestment = async (investmentData: Omit<Investment, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    await performCRUD(
      database.getInvestments,
      database.saveInvestments,
      { type: 'ADD_INVESTMENT', payload: investment },

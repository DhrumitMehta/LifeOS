...
const AnalyticsScreen = () => {
  ...
  const [fullScreenChartType, setFullScreenChartType] = useState<'boolean' | 'numeric-time' | 'numeric-day' | null>(null);
  ...
  // Landscape chart renderers for full screen
  const renderLandscapeBooleanChart = (habit: Habit) => {
    const data = getBooleanChartData(habit);
    const maxCount = Math.max(...data.map(d => d.count), 1);
    
    return (
      <View style={styles.landscapeChart}>
        <View style={styles.landscapeBarChart}>
          {data.map((item, index) => (
            <View key={index} style={styles.landscapeBarItem}>
              <View style={styles.landscapeBarContainer}>
                <View 
                  style={[
                    styles.landscapeBar, 
                    { 
              ...
              <Text style={styles.landscapeBarLabel}>{item.day}</Text>
              <Text style={styles.landscapeBarValue}>{item.count}</Text>
            </View>
          ))}
        </View>
      </View>
  ...
  const renderLandscapeLineChart = (habit: Habit) => {
    const data = getNumericTimeChartData(habit, dateRange);
    const maxValue = Math.max(...data.map(d => d.value), habit.maxValue || 10);
    
    return (
      <View style={styles.landscapeChart}>
        <View style={styles.landscapeLineChart}>
          {data.map((item, index) => {
            const nextItem = data[index + 1];
            const currentHeight = (item.value / maxValue) * 100;
            const nextHeight = nextItem ? (nextItem.value / maxValue) * 100 : currentHeight;
            ...
            return (
              <View key={index} style={styles.landscapeLineItem}>
                <View style={styles.landscapeLineContainer}>
                  <View 
                    style={[
                      styles.landscapeLinePoint,
                      { 
                        backgroundColor: habit.color,
                        bottom: `${currentHeight}%`
                      }
                ...
                <Text style={styles.landscapeLineLabel}>{item.date}</Text>
                <Text style={styles.landscapeLineValue}>{item.value}</Text>
              </View>
            );
          })}
        </View>
  ...
  const renderLandscapeNumericDayChart = (habit: Habit) => {
        ...
        <View style={styles.landscapeBarChart}>
          {data.map((item, index) => (
            <View key={index} style={styles.landscapeBarItem}>
              ...
              <Text style={styles.landscapeBarLabel}>{item.day}</Text>
              <Text style={styles.landscapeBarValue}>{item.average.toFixed(1)}</Text>
            </View>
          ))}
        </View>
      </View>
  ...
  return (
    ...
    >
              ...
              <View style={styles.chartContainer}>
                <View style={styles.chartHeader}>
                  ...
                  <Button
                    mode="outlined"
                    onPress={() => {
                      setFullScreenChartType('boolean');
              ...
              <View style={styles.chartContainer}>
                <View style={styles.chartHeader}>
                  ...
                  <Button
                    mode="outlined"
                    onPress={() => {
                      setFullScreenChartType('numeric-time');
                      setShowFullScreenChart(true);
                    }}
                    icon="fullscreen"
                    style={styles.fullScreenButton}
                ...
                <ScrollView 
                  ...
                  showsHorizontalScrollIndicator={false}
                >
                  {renderLineChart(selectedNumericHabit)}
                </ScrollView>
              </View>
            )}
          </Card.Content>
              ...
              <View style={styles.chartContainer}>
                <View style={styles.chartHeader}>
                  ...
                  <Button
                    mode="outlined"
                    onPress={() => {
                      setFullScreenChartType('numeric-day');
                      setShowFullScreenChart(true);
                    }}
                    icon="fullscreen"
                    style={styles.fullScreenButton}
      ...
      <Dialog 
        ...
        onDismiss={() => setShowFullScreenChart(false)}
        style={styles.fullScreenDialog}
      >
        <Dialog.Title>
          {fullScreenChartType === 'boolean' && selectedBooleanHabit?.name}
          {fullScreenChartType === 'numeric-time' && selectedNumericHabit?.name}
          {fullScreenChartType === 'numeric-day' && selectedNumericHabit2?.name}
        </Dialog.Title>
        <Dialog.Content style={styles.fullScreenContent}>
          {fullScreenChartType === 'boolean' && selectedBooleanHabit && (
            <View style={styles.fullScreenChart}>
              {renderLandscapeBooleanChart(selectedBooleanHabit)}
            </View>
          )}
          {fullScreenChartType === 'numeric-time' && selectedNumericHabit && (
            <View style={styles.fullScreenChart}>
              {renderLandscapeLineChart(selectedNumericHabit)}
            </View>
          )}
          {fullScreenChartType === 'numeric-day' && selectedNumericHabit2 && (
            <View style={styles.fullScreenChart}>
              {renderLandscapeNumericDayChart(selectedNumericHabit2)}
            </View>
          )}
        </Dialog.Content>
        <Dialog.Actions>
          <Button onPress={() => setShowFullScreenChart(false)}>Close</Button>
        </Dialog.Actions>
      </Dialog>
    </ScrollView>
  );
